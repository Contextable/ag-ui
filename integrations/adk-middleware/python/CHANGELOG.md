...
- **CHORE**: Update the default model for the live tests to `gemini-3.5-flash`
  - `gemini-2.0-flash` reached its shutdown date (2026-06-01) and `gemini-2.5-flash` is scheduled to shut down (2026-10-16), so the live/integration tests and examples now target the current stable flash GA, `gemini-3.5-flash`. A long-retired `gemini-1.5-pro` reference in an example was bumped to `gemini-2.5-pro` (the pro tier, not flash). The large file count is purely this model-string sweep — there are no library or runtime behavior changes.
