...
`HttpAgent` is a low-level HTTP transport implementation that provides direct control over AG-UI protocol communication. It handles the core HTTP communication, newline-delimited JSON (NDJSON) streaming, and event verification, serving as the foundation for higher-level agent implementations.
...
HttpAgent streams responses using newline-delimited JSON (NDJSON):
- Standard HTTP `application/json` responses with chunked transfer encoding
- Incremental decoding of each JSON object as it arrives
- Automatic recovery when malformed lines are encountered
- Compatible with OpenAI, Docker Model Runner, Ollama, and other OpenAI-style endpoints
